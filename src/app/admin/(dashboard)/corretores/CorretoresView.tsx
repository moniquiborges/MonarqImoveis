...
import { Users, Plus, Search, CheckCircle2, X, Edit2, Trash2, Award } from "lucide-react";
import {
  createAgent,
  deleteAgent,
  toggleAgentActive,
...
const emptyForm = {
  name: "",
  roleTitle: "Consultor Imobiliário",
  creci: "",
  email: "",
  phone: "",
};

export function CorretoresView({ initialAgents, initialError }: CorretoresViewProps) {
  ...
  const handleOpenEdit = (ag: AgentListItem) => {
    setEditingAgent(ag);
    setFormData({
      name: ag.name,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setFormError(result.error ?? "Não foi possível salvar o corretor.");
    }
  };

  const toggleActive = (ag: AgentListItem) => {
    setActionError(null);
    startTransition(async () => {
      const result = await toggleAgentActive(ag.id, !ag.active);
      if (result.success) {
        router.refresh();
      } else {
  ...
  const handleDelete = (id: string, name: string) => {
    if (!confirm(`Tem certeza que deseja excluir o corretor/consultor "${name}"?`)) return;
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setActionError(result.error ?? "Não foi possível excluir o corretor.");
      }
    });
  };
    ...
    <div className="flex flex-col gap-8 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-sm border border-areia/60 bg-white p-5 shadow-2xs">
          ...
          <p className="font-display text-3xl font-semibold text-mineral">{initialAgents.length}</p>
        ...
        <div className="rounded-sm border border-areia/60 bg-white p-5 shadow-2xs">
          ...
          <p className="font-display text-3xl font-semibold text-emerald-600">
            {initialAgents.filter((a) => a.active).length}
          </p>
        ...
        <div className="rounded-sm border border-areia/60 bg-white p-5 shadow-2xs">
          ...
          <p className="font-display text-3xl font-semibold text-terracota">
            {initialAgents.length > 0
              ? Math.round(
                  (initialAgents.filter((a) => a.creci.trim() !== "").length / initialAgents.length) * 100
                )
              : 0}
            %
          </p>
          ...
          <table className="w-full text-left text-xs text-graphite">
            ...
            <tbody className="divide-y divide-areia/30">
              {filteredAgents.length > 0 ? (
                    ...
                    <td className="p-4 text-right">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button
                          ...
                          disabled={isPending}
                          className="rounded-xs border border-areia/70 bg-offwhite/50 px-2.5 py-1 text-[11px] font-medium text-graphite hover:bg-offwhite transition-colors cursor-pointer disabled:opacity-50"
                ...
                <tr>
                  <td colSpan={6} className="p-8 text-center text-graphite/50 text-xs">
