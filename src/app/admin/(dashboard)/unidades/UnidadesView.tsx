...
const emptyForm = (defaultDevId: string) => ({
  developmentId: defaultDevId,
  unitNumber: "",
  tower: "Torre A",
  floor: "1",
  area: "",
...
export function UnidadesView({ initialUnits, developments, initialError }: UnidadesViewProps) {
  ...
  const [newUnit, setNewUnit] = useState(emptyForm(developments[0]?.id ?? ""));

  const filteredUnits = initialUnits.filter((u) => {
    if (selectedDev !== "all" && u.developmentId !== selectedDev) return false;
  ...
  const totalVGV = filteredUnits.reduce((acc, u) => acc + (u.price ?? 0), 0);
  ...
  const handleCreateUnit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      setNewUnit(emptyForm(developments[0]?.id ?? ""));
      setIsModalOpen(false);
      router.refresh();
    } else {
      setFormError(result.error ?? "Não foi possível cadastrar a unidade.");
    }
  };

  const handleChangeStatus = (id: string, newStatus: UnitStatus) => {
    setActionError(null);
  ...
  const handleDeleteUnit = (id: string) => {
    if (!confirm("Deseja realmente remover esta unidade do espelho de vendas?")) return;
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setActionError(result.error ?? "Não foi possível excluir a unidade.");
      }
    });
  };
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="font-display text-2xl md:text-3xl text-mineral">Gestão de Unidades</h1>
        ...
        <button
          ...
          className="focus-ring inline-flex items-center justify-center gap-2 rounded-xs bg-mineral px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-offwhite hover:bg-mineral-light transition-colors shadow-xs cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          title={developments.length === 0 ? "Cadastre um empreendimento primeiro" : undefined}
        >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        ...
        <div className="rounded-sm border border-areia/60 bg-white p-5 shadow-2xs">
          ...
          <p className="font-display text-2xl font-semibold text-graphite">{formatBRL(totalVGV)}</p>
                ...
                <tr key={u.id} className="hover:bg-offwhite/40 transition-colors">
                  <td className="p-4">
                    ...
                    <span className="text-[11px] text-graphite/50">
                      {u.tower}
                      {u.tower && u.floor != null ? " · " : ""}
                      {u.floor != null ? `${u.floor}° Andar` : ""}
                    </span>
                  </td>

                  <td className="p-4 font-mono font-medium text-graphite">{u.unitNumber}</td>

                  <td className="p-4 text-graphite/80">
                    {u.bedrooms ?? "—"} dorms ({u.suites ?? "—"} suítes) · {u.parking ?? "—"} vagas
                  </td>

                  <td className="p-4 font-semibold text-graphite">{u.area} m²</td>

                  <td className="p-4 font-semibold text-mineral text-sm">
                    {u.price ? formatBRL(u.price) : "Sob consulta"}
                  </td>
                  ...
                  <td className="p-4">
                    <select
                      value={u.status}
                      disabled={isPending}
                      onChange={(e) => handleChangeStatus(u.id, e.target.value as UnitStatus)}
                      className={`focus-ring rounded-xs px-2.5 py-1 text-[11px] font-semibold cursor-pointer disabled:opacity-50 ${
                        u.status === "disponivel"
                  ...
                  <td className="p-4 text-right">
                    <button
                      ...
                      disabled={isPending}
                      className="rounded-xs p-1.5 text-graphite/40 hover:text-rose-600 transition-colors cursor-pointer disabled:opacity-40"
          ...
          <div className="w-full max-w-lg rounded-sm border border-areia/60 bg-white p-6 shadow-2xl space-y-5 animate-scale-in">
            ...
            <form onSubmit={handleCreateUnit} className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={newUnit.developmentId}
                  onChange={(e) => setNewUnit({ ...newUnit, developmentId: e.target.value })}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-graphite mb-1">Torre / Bloco</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-graphite mb-1">Andar</label>
                  <input
                    type="number"
                    value={newUnit.floor}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-graphite mb-1">Status Inicial</label>
                  <select
                    value={newUnit.status}
                    onChange={(e) =>
                      setNewUnit({ ...newUnit, status: e.target.value as UnitStatus })
                    }
                    className="focus-ring w-full rounded-xs border border-areia/70 bg-offwhite/30 px-3 py-2 text-xs text-graphite cursor-pointer"
                  >
                    <option value="disponivel">Disponível</option>
