...
import { primaryNav, cgDefaultCategories } from "./nav-data";
import { useFavorites } from "@/contexts/FavoritesContext";
import { fetchCampoGrandePropertyTypes } from "@/lib/services/propertyService";
...
export function Header() {
  ...
  const [cgChildren, setCgChildren] = useState(cgDefaultCategories);

  useEffect(() => {
    fetchCampoGrandePropertyTypes().then((types) => {
      if (types && types.length > 0) {
        const dynamicLinks = [
          { label: "Todos os Imóveis", href: "/imoveis/campo-grande" },
          ...types.map((t) => ({
            label: TYPE_DISPLAY_LABELS[t] || t,
            href: `/imoveis/campo-grande?tipo=${encodeURIComponent(t)}`,
          })),
        ];
        setCgChildren(dynamicLinks);
      }
    });
  }, []);
    ...
    <header className="sticky top-0 z-40 border-b border-graphite/8 bg-offwhite/95 backdrop-blur-md">
      <div className="mx-auto flex h-20 w-full max-w-[1480px] items-center justify-between px-5 md:px-8 lg:px-12">
        ...
        <nav className="hidden items-center gap-4 xl:gap-6 2xl:gap-7 lg:flex">
          {primaryNav.map((item) => {
            const children = item.label === "Campo Grande" ? cgChildren : item.children;

            return (
              <div
                key={item.href}
                className="relative py-2"
                onMouseEnter={() => children && setOpenDropdown(item.label)}
