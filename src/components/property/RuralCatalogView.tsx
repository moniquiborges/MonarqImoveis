...
interface Props {
  initialProperties: RuralProperty[];
}

export function RuralCatalogView({ initialProperties }: Props) {
  const [dbProps, setDbProps] = useState<RuralProperty[]>(initialProperties);

  useEffect(() => {
    fetchRuralProperties().then((props) => {
      if (props && props.length > 0) {
        setDbProps(props);
        saveStoredRuralProperties(props);
      }
    });
  }, []);
  ...
  const ruralProperties = dbProps && dbProps.length > 0 ? dbProps : initialProperties;
  ...
  }, [ruralProperties, selectedState, selectedActivity, selectedType, searchQuery]);
